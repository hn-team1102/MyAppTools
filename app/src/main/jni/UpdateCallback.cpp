      ...
      Int32 *newData, Int32 *newProps, UInt32 *indexInArchive)
{
  ...
  if (newData != nullptr) *newData = BoolToInt(up.NewData);
  if (newProps != nullptr) *newProps = BoolToInt(up.NewProps);
  if (indexInArchive != nullptr)
  {
    *indexInArchive = (UInt32)-1;
    if (up.ExistInArchive())
      *indexInArchive = (ArcItems == nullptr) ? up.ArcIndex : (*ArcItems)[up.ArcIndex].IndexInServer;
  }
  return S_OK;
  COM_TRY_END
}
